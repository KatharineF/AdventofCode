...
def fill_schemaDict(schemaDict, x, num, p1, p2, line, length):
    schemaDict[x] = {"num": num, "p1": p1, "p2": p2, "line": line, "length": length} 
    return schemaDict
...
for index in schemaDict:
    ...
    for x in lineRange:
        ...
        for y in pointerRange:
            #print(x,y)
            #print(schema[x][y])
            if not schema[x][y].isnumeric() and not ".": #<--- fix this 
